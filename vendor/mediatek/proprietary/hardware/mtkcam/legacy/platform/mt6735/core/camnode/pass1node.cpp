...
dequeLoop()
{
    ...
    for( iter = dequeBufInfo.mvOut.begin(); iter != dequeBufInfo.mvOut.end(); iter++ )
    {
		...
		if(iter->mMetaData.mRawType == 1 )
		{
			...
			MY_LOGD("iris, buffersize.width(%d), imageformat(%d),planecount(%d)", iter->mBuffer->getImgSize().w,iter->mBuffer->getImgFormat(),iter->mBuffer->getPlaneCount());
			MY_LOGD("iris, buffersize.height(%d), getstrides(%d)", iter->mBuffer->getImgSize().h,iter->mBuffer->getBufStridesInBytes(0));
			saveiris(ptr,iter->mBuffer->getBufStridesInBytes(0)*iter->mBuffer->getImgSize().h);
		}
			//add iris unlock
...
configFrame(MUINT32 const magicNum)
{
    ...
    if(mbCfgRrzo) // if rrzo is used
    {
        ...
        FrameParam currFrameParam = FrameParam(
                                        ...
                                        pass1OutSize,
                                        pass1InCrop);
	MY_LOGD("configFrame rrzo w(%d),h(%d)",pass1OutSize.w,pass1OutSize.h);
    ...
    if(mbCfgImgo) // if only use imgo
    {
        ...
        FrameParam currFrameParam = FrameParam(
                                        ...
                                        pass1OutSize,
                                        pass1InCrop);
	MY_LOGD("configFrame imgo w(%d),h(%d)",pass1OutSize.w,pass1OutSize.h);
        currQFrameParam.mvOut.push_back(currFrameParam);
    }
    //
    ...
    //if (mInitCfg.muRawType == SENSOR_DYNAMIC_PURE_RAW)
    {
        MY_LOGD("iris currQFrameParam.mMagicNum=%d, purerawEnabled=%d", currQFrameParam.mMagicNum, purerawEnabled);
    }

...
onInit()
{
    ...
    MUINT32 scenario, type;
    mpIspSyncCtrlHw->getSensorInfo(scenario, sensorWidth, sensorHeight, type);
    MY_LOGD("sensor w(%d), h(%d)", sensorWidth, sensorHeight);
    ret = MTRUE;
lbExit:
    FUNC_END;
    return ret;
...
handleP2Done(QParams& rParams)
{
    ...
    for( iterOut = rParams.mvOut.begin() ; iterOut != rParams.mvOut.end() ; iterOut++ )
    {
	 ...
	 MY_LOGD("Buf node Type is %d",nodeDataType);
            // fn unlock
			unsigned char* ptr = (MUINT8*)iterOut->mBuffer->getBufVA(0);
			if(iterOut->mBuffer->getImgFormat()==17)
			//if(nodeDataType==PASS2_PRV_DST_0)
			{
                MUINT32 width, height, strides;
                if (sensorWidth > iterOut->mBuffer->getImgSize().w)
                    width = iterOut->mBuffer->getImgSize().w;
                else
                    width = sensorWidth;
                if (sensorHeight > iterOut->mBuffer->getImgSize().h)
                    height = iterOut->mBuffer->getImgSize().h;
                else
                    height = sensorHeight;
                strides = sensorWidth*10/8;
                strides = ((strides + 3)/4);
                strides = strides*4;
                readirisaddr((MUINT8*)iterOut->mBuffer->getBufVA(0), width, height, strides);
			}
			// fn unlock
...
readirisaddr(MUINT8* irisbuff,int w,int h,int strides)
{
    ...
    //if you want unpack, you need to uncomments following two line
	//covertoraw(irisread,rmbbuff,w,h,strides);
	//memcpy(irisbuff,rmbbuff,w*h);

    //for copy directly instead of unpack
    //strides for 1080 is 1352, for 1920 is 2400
	memcpy(irisbuff, irisread, h*strides);
    MY_LOGD("readiris, copied, w:%d, h:%d, strides:%d", w, h, strides);
}
	// fn unlock
/*******************************************************************************
*
